...
void CInputManager::Initialize()
{
	...
	for (_uint i = 0; i < KEY_MAX; ++i)
	{
		m_keyUp.set(i, false);
		m_keyDown.set(i, false);
	}

	position = _float3(0, 0, 0);

	wheelStatus = _float3(0.0f, 0.0f, 0.0f);
	wheelOldStatus = _float3(0.0f, 0.0f, 0.0f);
	wheelMoveValue = _float3(0.0f, 0.0f, 0.0f);
...
void CInputManager::Update()
{
	...
	wheelMoveValue = { wheelStatus.x - wheelOldStatus.x,
		wheelStatus.y - wheelOldStatus.y,
		wheelStatus.z - wheelOldStatus.z };
...
void CInputManager::InputProc(const HWND hWnd, const UINT message, const WPARAM wParam, const LPARAM lParam)
{
	if (nullptr == m_hWnd)
		m_hWnd = hWnd;
...
bool CInputManager::IsKeyDown(_uint key)
{
...
bool CInputManager::IsKeyUp(_uint key)
{
	if (GetAsyncKeyState(key) & 0x8000)
	{
...
bool CInputManager::IsKeyPressed(_uint key)
{
	if (GetAsyncKeyState(key) & 0x8000)
		return true;
	return false;
...
bool CInputManager::IsKeyToggled(_uint key)
{
...
bool CInputManager::IsMouseDown(DWORD mouse)
{
	return buttonMap[mouse] == BUTTON_INPUT_STATUS_DOWN;
}

bool CInputManager::IsMouseUp(DWORD mouse)
{
	return buttonMap[mouse] == BUTTON_INPUT_STATUS_UP;
}

bool CInputManager::IsMousePressed(DWORD mouse)
{
	return buttonMap[mouse] == BUTTON_INPUT_STATUS_PRESS;
}

const _float3 CInputManager::GetMouseMoveValue() const
{
